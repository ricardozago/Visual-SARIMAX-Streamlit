...
def main():
    ...
    c_nome_data, c_nome_qty, c_is_data = st.beta_columns(3)
    ...
    is_data = c_is_data.checkbox('O index é uma data?', value=True, key="is_data")
    ...
    if df is not None:
        fig = plot_grafico_1(df, nome_data, nome_qty, is_data)
        st.plotly_chart(fig)
        ...
        tests_adf_autocorr(y[nome_qty], 'Testes estatísticos para a série sem diferenciação')
        ...
        **Sazonais:**
        ...
        if par_ARIMA["n"] == 1:
            ...
            exogenas = y.drop(nome_qty, axis = 1).columns

            if len(exogenas) > 0:
                st.markdown('''
                **Exógenas (X):**
                ''')
                selected_exogens = st.multiselect('Selecione as exógenas a serem utilizadas no modelo:', exogenas)
            else:
                selected_exogens = []
            ...
            if len(selected_exogens) == 0:
                ...
                if st.button('Estimar com auto.arima'):
                    if par_ARIMA["n"] > 0:
                        model_auto_arima = pm.auto_arima(y_train[nome_qty], seasonal=True, m=par_ARIMA["n"], with_intercept = False, trend = 'n')
                    else:
                        model_auto_arima = pm.auto_arima(y_train[nome_qty], seasonal=False, with_intercept = False, trend = 'n')
                ...
                st.markdown('Não é possível utilizar o autorima quando são utilizadas variáveis exógenas')

            ##################################################
            # Refaz os testes ADF e ACF/PACF caso exista diferenciação
            ##################################################
            if par_ARIMA["d"] > 0:
                tests_adf_autocorr(y_train[nome_qty].diff(periods=par_ARIMA["d"])[par_ARIMA["d"]:], 'Testes estatísticos para a série com d = {}'.format(par_ARIMA["d"]))

            ##################################################
            # Treina modelo
            ##################################################
            if len(selected_exogens) == 0:
                model = ARIMA(y_train[nome_qty], order=(par_ARIMA["p"], par_ARIMA["d"], par_ARIMA["q"]), 
                    seasonal_order=(par_ARIMA["P"], par_ARIMA["D"], par_ARIMA["Q"], par_ARIMA["n"]), trend = par_ARIMA["trend"])
            else:
                y_train_exogs = y_train.drop(nome_qty, axis = 1)[selected_exogens]
                y_test_exogs = y_test.drop(nome_qty, axis = 1)[selected_exogens]
                model = ARIMA(y_train[nome_qty], exog = y_train_exogs, order=(par_ARIMA["p"], par_ARIMA["d"], par_ARIMA["q"]), 
                    seasonal_order=(par_ARIMA["P"], par_ARIMA["D"], par_ARIMA["Q"], par_ARIMA["n"]), trend = par_ARIMA["trend"])
            model_fit = model.fit()
            ...
            if len(selected_exogens) == 0:
                ...
                y_pred = model_fit.forecast(steps=y_test_exogs.shape[0], exog = y_test_exogs)
                y_pred.index = y_pred.index + 1

            ##################################################
            # Plota Forecast
            ##################################################
            fig_forecast = plot_grafico_projecao(y_train[nome_qty], y_test[nome_qty], y_pred, nome_data, nome_qty, is_data)
            ...
            if y_test is not None:
                ...
                with metricas_analise:
                    ...
                    metric_MAPE = mean_absolute_percentage_error(y_test[nome_qty], y_pred_metrics)
                    metric_RMSE = np.sqrt(mean_absolute_error(y_test[nome_qty], y_pred_metrics))
                    st.markdown(f'''
                    **Métricas:**  
            ...
            with resid_analise:
                ...
                acf_resi = ts.acf(residuos, nlags = min(int(residuos.shape[0]/2-1), 25), alpha = .05)
                c_acf_resi.plotly_chart(plot_auto_correlation(acf_resi, 25, "ACF dos resíduos"))
                pacf_resi = ts.pacf(residuos, nlags = min(int(residuos.shape[0]/2-1), 25), alpha = .05)
            ...
            ##################################################
            if y_test is not None:
                df_projecao = pd.concat([y_train[nome_qty], y_test[nome_qty], y_pred], axis = 1).fillna(0).reset_index()
                df_projecao.columns = ["Data", "Treinamento", "Teste", "Projeção"]
            else:
                df_projecao = pd.concat([y_train[nome_qty], y_pred[nome_qty]], axis = 1).fillna(0).reset_index()
...
def tests_adf_autocorr(y, texto):
    ...
    with statistical_tests:
        ...
        acf = ts.acf(y, nlags = min(int(y.shape[0]/2-1), 25), alpha = .05)
        c_acf.plotly_chart(plot_auto_correlation(acf, 25, "ACF"))
        pacf = ts.pacf(y, nlags = min(int(y.shape[0]/2-1), 25), alpha = .05)
